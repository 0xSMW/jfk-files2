      ...
      <div className="p-4 h-full overflow-y-auto">
        ...
        {isDocument && (
          <>
            ...
            <div className="mb-4">
              <h4 className="text-sm font-medium text-foreground mb-1">Tags</h4>
              <div className="flex flex-wrap gap-1">
                {(metadata as Document).tags?.map(tag => (
                  <Link
                    key={tag}
                    href={`/visualization?entity=${encodeURIComponent(tag)}`}
                    className="inline-flex items-center rounded-full bg-muted px-2.5 py-0.5 text-xs font-medium text-foreground hover:bg-muted/80"
                  >
                    {tag}
                  </Link>
        ...
        {isEntity && (
          <>
            ...
            <div className="mb-4">
              <h4 className="text-sm font-medium text-foreground mb-1">Significance</h4>
              <p className="text-sm text-muted-foreground">
            ...
            <div className="mb-4">
              <h4 className="text-sm font-medium text-foreground mb-1">Related to {(metadata as Entity).document_count || 0} documents</h4>

              {(metadata as Entity).key_connections && (metadata as Entity).key_connections.length > 0 && (
                <div className="mt-2">
                  <h4 className="text-sm font-medium text-foreground mb-1">Key connections</h4>
                  <div className="flex flex-wrap gap-1">
        ...
        <div className="mt-6 pt-4 border-t border-border">
          <h4 className="text-sm font-medium text-foreground mb-2">Connected In This Graph</h4>
          <div className="space-y-2">
            ...
            {graphData.links.filter(link =>
              ...
              <p className="text-sm text-muted-foreground">No connections in this graph view</p>
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="rounded-xl border bg-card overflow-hidden">
      <div className="p-4 border-b">
        <h2 className="text-lg font-semibold text-foreground">Document Relationships</h2>
        <p className="text-sm text-muted-foreground">
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <div className={`flex-1 ${showDetailPanel ? 'md:w-7/12' : 'w-full'}`}>
          {loading ? (
            ...
            <div className="flex items-center justify-center h-80 p-4 text-center">
              <div>
                <p className="text-muted-foreground mb-2">No relationships found for this document</p>
                <p className="text-sm text-muted-foreground">This document does not have any tagged entities or connections</p>
              </div>
            </div>
          ) : (
            <div className="relative">
              ...
              <div className="absolute bottom-4 right-4 flex space-x-2">
                <button
                  ...
                  className="bg-background text-foreground px-2 py-1 rounded text-xs border hover:bg-muted"
                >
        ...
        {showDetailPanel && (
          <div className="md:w-5/12 border-t md:border-t-0 md:border-l bg-muted/50">
            {renderDetailPanel()}
          </div>
        )}
      </div>
